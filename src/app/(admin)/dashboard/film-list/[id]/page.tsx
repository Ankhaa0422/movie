...
import { Icon, Input, Select, Date, Number, showLoader } from "@/components"
...
import { addFilm, uploadPoster, getFilmInfo, updateFilm } from '@/server-actions';
...
export default function Page ({ params }: {params: {id:any}}) {
    ...
    React.useEffect(() => {
        ...
        async function getData() {
            ...
            setState(result)
            setFilmScreenShots(result.Images)
            setLinks(result.Links)
            setMnPlot(result.MnPlot)
            setEnPlot(result.Plot)
            // setFilmData(result)
            // setActiveImage({
            //     src: result.Images[0],
            //     index: 0
    ...
    const onPublish = async () => {
        ...
        data['Links'] = links
        const response = await updateFilm(params.id ,data)
        if(response) {
            router.back()
        }
        showLoader(false)
    ...
    return <div ref={focusTrapRef} className={`w-[calc(100vw-150px)] h-[calc(100vh-100px)] flex flex-row`}>
        ...
        <div className="w-[300px] min-w-[300px] h-full px-5 py-3 flex flex-col gap-4 overflow-auto">
            <button className='py-2 bg-emerald-600 rounded font-semibold' onClick={onPublish}>
                Edit
            </button>
